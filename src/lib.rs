...
fn main() -> Result<(), String> {
    ...
    if cfg!(target_arch="wasm32"){
        window_config = window_config.maximized();
    }
...
fn load_pipeline(gfx: &mut Graphics, vert: &[u8], frag: &[u8]) -> Result<Pipeline> {
    let pipeline = gfx
        .create_pipeline()
        .from_raw(vert, frag)
        .build()
...
fn setup(gfx: &mut Graphics) -> State {
    ...
    let vert = include_bytes!("shader.vert");
    let frag = include_bytes!("shader.frag");

    let pipeline = match load_pipeline(gfx, vert, frag) {
        Ok(pipeline) => pipeline,
        Err(err) => {
            println!("Error compiling shaders: {}", err);
            panic!("{}", err);
        }
    };
...
fn draw(app: &mut App, gfx: &mut Graphics, plugins: &mut Plugins, state: &mut State) {
    ...
    if state.must_reload_shaders {
        ...
        let vert = std::fs::read("src/shader.vert").unwrap();
        let frag = std::fs::read("src/shader.frag").unwrap();

        match load_pipeline(gfx, &vert, &frag) {
            Ok(pipeline) => {
